...
| `netlify/functions/parse-jd.js` | AI 识别云函数（判定是否为JD + 抽取字段） | 需要时改 |

## 更新流程（每次发版）

1. 修改 `jobhunt.html`
2. 运行 `python3 build_pwa.py`，把生成的 `jobhunt-app/index.html` 覆盖到仓库对应位置
3. `sw.js` 第一行缓存版本号 +1（如 `jobhunt-v9` → `jobhunt-v10`）
4. 提交并推送 → Netlify 自动部署（约 1 分钟）
5. 无痕窗口打开线上地址确认生效（自己设备缓存需刷新两次）

⚠ 注意 Netlify 的 Base directory 设置需与仓库内文件实际位置一致。
...
改环境变量后需重新部署一次才生效。识别行为：非 JD 内容会返回提醒而非强行填表。
